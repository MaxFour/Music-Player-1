...
import com.android.music.model.TagUpdate;
        ...
        for (int i = 0; i < paths.size(); i++) {
            ...
            try {
                ...
                if (tagUpdate.hasChanged()) {
                    ...
                    if (MPlayerUtils.hasLollipop() && TaggerUtils.requiresPermission(paths)) {
                        temp = new File(MusicApplication.getInstance().getFilesDir(), orig.getName());
                        tempFiles.add(temp);
                    ...
                    if (MPlayerUtils.hasLollipop() && requiresPermission && temp != null) {
                        DocumentFile documentFile = documentFiles.get(i);
                        if (documentFile != null) {
                            ParcelFileDescriptor pfd = MusicApplication.getInstance().getContentResolver().openFileDescriptor(documentFile.getUri(), "w");
                            if (pfd != null) {
