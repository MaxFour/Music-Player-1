...
import com.android.music.model.InclExclItem;
import com.android.music.model.Playlist;
import com.android.music.model.Song;
import com.android.music.sql.databases.InclExclDbOpenHelper;
...
public class DataManager {
    ...
    public Observable<List<Song>> getAllSongsRelay() {
        if (allSongsSubscription == null || allSongsSubscription.isDisposed()) {
            allSongsSubscription = SqlBriteUtils.createObservableList(MusicApplication.getInstance(), Song::new, Song.getQuery()).subscribe(allSongsRelay, error -> LogUtils.logException(TAG, "getAllSongsRelay threw error", error));
        }
    ...
    public Observable<List<Genre>> getGenresRelay() {
        if (genresSubscription == null || genresSubscription.isDisposed()) {
            genresSubscription = SqlBriteUtils.createObservableList(MusicApplication.getInstance(), Genre::new, Genre.getQuery())
    ...
    public Observable<List<Playlist>> getPlaylistsRelay() {
        if (playlistsSubscription == null || playlistsSubscription.isDisposed()) {
            playlistsSubscription = SqlBriteUtils.createObservableList(MusicApplication.getInstance(), Playlist::new, Playlist.getQuery())
    ...
    public BriteDatabase getInclExclDatabase() {
        if (inclExclDatabase == null) {
            inclExclDatabase = new SqlBrite.Builder().build()
                    .wrapDatabaseHelper(new InclExclDbOpenHelper(MusicApplication.getInstance()), Schedulers.io());
        }
