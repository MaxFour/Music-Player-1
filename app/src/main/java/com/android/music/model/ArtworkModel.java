    ...
    public static String getTypeString(@ArtworkProvider.Type int type) {
        switch (type) {
            case ArtworkProvider.Type.MEDIA_STORE:
                return MusicApplication.getInstance().getString(R.string.artwork_type_media_store);
            case ArtworkProvider.Type.TAG:
                return MusicApplication.getInstance().getString(R.string.artwork_type_tag);
            case ArtworkProvider.Type.FOLDER:
                return "Folder";
            case ArtworkProvider.Type.LAST_FM:
                return MusicApplication.getInstance().getString(R.string.artwork_type_lastfm);
            case ArtworkProvider.Type.ITUNES:
                return MusicApplication.getInstance().getString(R.string.artwork_type_itunes);
        }
        return null;
    }
